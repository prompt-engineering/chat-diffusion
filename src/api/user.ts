...
export async function login(key: string, token: string) {
  if (EdgeUser.isClientSideOpenAI()) return EdgeUser.saveApiKey(key, token);
  const response = await fetch("/api/chatgpt/user", {
    method: "POST",
    body: JSON.stringify({
      action: "login",
...
export async function isLoggedIn(hashedKey?: string) {
  if (typeof window !== "undefined" && typeof document !== "undefined") {
    ...
    if (EdgeUser.isClientSideOpenAI())
      return EdgeUser.getApiKey() && EdgeUser.getToken() ? true : false;
  ...
  //   new URL("/api/chatgpt/verify", new URL(urlStr)),
  //   {
  //     method: "POST",
  //     body: hashedKey ?? "NOPE",
